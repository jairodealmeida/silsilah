...
@can('edit', $race)
    <div class="pull-right">
        {{ link_to_route('races.edit', trans('race.edit'), $race, ['class' => 'btn btn-warning']) }}
    </div>
@endcan
<h2 class="page-header">
    {{ $race->husband->name }} & {{ $race->wife->name }} <small>{{ trans('race.detail') }}</small>
</h2>

@include('races.partials.stat')
<br>
<h4 class="page-header">{{ trans('user.childs') }} & {{ trans('user.grand_childs') }}</h4>
@if ($race->childs->isEmpty())
    <i>{{ trans('app.childs_were_not_recorded') }}</i>
@else
    <?php $no = 0; ?>
    @foreach($race->childs->chunk(4) as $chunkedChild)
    <div class="row">
